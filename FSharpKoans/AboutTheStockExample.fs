...
module ``about the stock example`` =
    ...
    let YouGotTheAnswerCorrect() =
        let parseLine (line:string) =
            ...
            let openingPrice = parseDouble splitLine.[1]
            let closingPrice = parseDouble splitLine.[4]

            (date, openingPrice, closingPrice)

        let openingAndClosingPricesPerDay = stockData
                                            |> Seq.skip 1
                                            |> Seq.map parseLine

        let getOpenClosePriceDiff (_, openingPrice, closingPrice) = openingPrice - closingPrice
        let getDate (date, _, _) = date

        let result =  openingAndClosingPricesPerDay
                        |> Seq.maxBy (getOpenClosePriceDiff >> abs)
                        |> getDate
